<template>
    <div class="modal-stack">
        <b-modal ref="ctxModal" id="ctx-modal" hide-header hide-footer centered rounded size="sm"
            body-class="list-group-flush p-0 rounded">

            <div class="list-group text-center">
                <div v-if="status.visibility !== 'archived'" class="list-group-item d-flex p-0 m-0">
                    <div class="border-right p-2 w-50">
                        <a v-if="status" class="menu-option" :href="status.url" @click.prevent="ctxMenuGoToPost()">
                    ...
                    <div class="p-2 flex-grow-1">
                        <a v-if="status" class="menu-option" :href="status.account.url"
                ...
                <template v-if="ctxMenuRelationship">
                    <a v-if="ctxMenuRelationship.following" class="list-group-item menu-option text-danger" href="#"
                    ...
                    <div v-else class="d-flex">
                        <div class="p-3 border-right w-50 text-center">
                            <a class="small menu-option text-muted" href="#" @click.prevent="handleMute">
                                <div class="action-icon-link-inline">
                                    <div class="icon"><i class="far"
                                            :class="[ctxMenuRelationship.muting ? 'fa-eye' : 'fa-eye-slash']"></i>
                                    </div>
                        ...
                        <div class="p-3 w-50">
                            <a class="small menu-option text-danger" href="#" @click.prevent="handleBlock">
                                <div class="action-icon-link-inline">
                ...
                <a v-if="status.visibility !== 'archived'" class="list-group-item menu-option" href="#"
                    @click.prevent="ctxMenuShare()">
                    {{ $t('common.share') }}
                </a>

                <a v-if="status && profile && profile.is_admin == true && status.visibility !== 'archived'"
                    class="list-group-item menu-option" href="#" @click.prevent="ctxModMenuShow()">
                    {{ $t('menu.moderationTools') }}
                </a>

                <a v-if="status && status.account.id != profile.id" class="list-group-item menu-option text-danger"
                    href="#" @click.prevent="ctxMenuReportPost()">
                    {{ $t('menu.report') }}
                </a>

                <a v-if="status && profile.id == status.account.id && status.visibility !== 'archived'"
                    class="list-group-item menu-option text-danger" href="#" @click.prevent="archivePost(status)">
                    {{ $t('menu.archive') }}
                </a>

                <a v-if="status && profile.id == status.account.id && status.visibility == 'archived'"
                    class="list-group-item menu-option text-danger" href="#" @click.prevent="unarchivePost(status)">
                    {{ $t('menu.unarchive') }}
                </a>
                <a v-if="status && profile.id == status.account.id && !status.pinned"
                    class="list-group-item menu-option text-danger" href="#" @click.prevent="pinPost(status)">
                    {{ $t('menu.pin') }}
                </a>

                <a v-if="status && profile.id == status.account.id && status.pinned"
                    class="list-group-item menu-option text-danger" href="#" @click.prevent="unpinPost(status)">
                    {{ $t('menu.unpin') }}
                </a>

                <a v-if="status && profile.id == status.account.id && status.visibility !== 'archived'"
                    class="list-group-item menu-option text-danger" href="#" @click.prevent="editPost(status)">
                    Edit
                </a>

                <a v-if="status && (profile.is_admin || profile.id == status.account.id) && status.visibility !== 'archived'"
                    class="list-group-item menu-option text-danger" href="#" @click.prevent="deletePost(status)">
                ...
                <a class="list-group-item menu-option text-lighter" href="#" @click.prevent="closeCtxMenu()">
                    {{ $t('common.cancel') }}
                </a>
            </div>
        </b-modal>

        <b-modal ref="ctxModModal" id="ctx-mod-modal" hide-header hide-footer centered rounded size="sm"
            body-class="list-group-flush p-0 rounded">

            <div class="list-group text-center">
                <p class="py-2 px-3 mb-0">
                <div class="text-center menu-option text-danger">
                    {{ $t('menu.moderationTools') }}
                </div>
                ...
                <a class="list-group-item menu-option" href="#" @click.prevent="moderatePost(status, 'unlist')">
                    {{ $t('menu.unlistFromTimelines') }}
                </a>

                <a v-if="status.sensitive" class="list-group-item menu-option" href="#"
                    @click.prevent="moderatePost(status, 'remcw')">
                    {{ $t('menu.removeCW') }}
                </a>

                <a v-else class="list-group-item menu-option" href="#" @click.prevent="moderatePost(status, 'addcw')">
                    {{ $t('menu.addCW') }}
                </a>

                <a class="list-group-item menu-option" href="#" @click.prevent="moderatePost(status, 'spammer')">
                    {{ $t('menu.markAsSpammer') }}<br />
                    <span class="small">{{ $t('menu.markAsSpammerText') }}</span>
                </a>

                <a class="list-group-item menu-option text-lighter" href="#" @click.prevent="ctxModMenuClose()">
                    {{ $t('common.cancel') }}
                </a>
            </div>
        </b-modal>

        <b-modal ref="ctxModOtherModal" id="ctx-mod-other-modal" hide-header hide-footer centered rounded size="sm"
            body-class="list-group-flush p-0 rounded">
            <div class="list-group text-center">
                <p class="py-2 px-3 mb-0">
                ...
                <div class="list-group-item rounded cursor-pointer font-weight-bold" @click="confirmModal()">Unlist
                    Posts</div>
                <div class="list-group-item rounded cursor-pointer font-weight-bold" @click="confirmModal()">Moderation
                    Log
                </div>
                <div class="list-group-item rounded cursor-pointer text-lighter" @click="ctxModOtherMenuClose()">{{
                    $t('common.cancel') }}</div>
            </div>
        </b-modal>

        <b-modal ref="ctxShareModal" id="ctx-share-modal" title="Share" hide-footer hide-header centered rounded
            size="sm" body-class="list-group-flush p-0 rounded text-center">
            <a class="list-group-item menu-option" href="#" @click.prevent="ctxMenuCopyLink()">
                {{ $t('common.copyLink') }}
            </a>

            <a v-if="status && status.local == true && !status.in_reply_to_id" class="list-group-item menu-option"
                @click.prevent="ctxMenuEmbed()">
                {{ $t('menu.embed') }}
            </a>

            <a class="list-group-item menu-option text-lighter" href="#" @click.prevent="closeCtxShareMenu()">
                {{ $t('common.cancel') }}
            </a>
        </b-modal>

        <b-modal ref="ctxEmbedModal" id="ctx-embed-modal" hide-header hide-footer centered rounded size="md"
            body-class="p-2 rounded">
            <div>
                <div class="form-group">
                    <textarea class="form-control disabled text-monospace" rows="8"
                        style="overflow-y:hidden;border: 1px solid #efefef; font-size: 12px; line-height: 18px; margin: 0 0 7px;resize:none;"
                        v-model="ctxEmbedPayload" disabled=""></textarea>
                </div>
                <div class="form-group pl-2 d-flex justify-content-center">
                    <div class="form-check mr-3">
                        <input class="form-check-input" type="checkbox" v-model="ctxEmbedShowCaption"
                            :disabled="ctxEmbedCompactMode == true">
                        <label class="form-check-label font-weight-light">
                            {{ $t('menu.showCaption') }}
                        </label>
                    </div>
                    <div class="form-check mr-3">
                        <input class="form-check-input" type="checkbox" v-model="ctxEmbedShowLikes"
                            :disabled="ctxEmbedCompactMode == true">
                        <label class="form-check-label font-weight-light">
                            {{ $t('menu.showLikes') }}
                        </label>
                ...
                <div class="pl-2 d-flex justify-content-center">
                    <div class="btn-group btn-group-sm">
                        <button class="btn" :class="[uiColorScheme == 'system' ? 'btn-primary' : 'btn-outline-primary']"
                            @click="toggleUi('system')">
                            {{ $t('appearance.auto') }}
                        </button>
                        <button class="btn" :class="[uiColorScheme == 'light' ? 'btn-primary' : 'btn-outline-primary']"
                            @click="toggleUi('light')">
                            {{ $t('appearance.lightMode') }}
                        </button>
                        <button class="btn" :class="[uiColorScheme == 'dark' ? 'btn-primary' : 'btn-outline-primary']"
                            @click="toggleUi('dark')">
                ...
                <hr>
                <button
                    :class="copiedEmbed ? 'btn btn-primary btn-block btn-sm py-1 font-weight-bold disabed' : 'btn btn-primary btn-block btn-sm py-1 font-weight-bold'"
                    @click="ctxCopyEmbed" :disabled="copiedEmbed">
                    {{ copiedEmbed ? 'Embed Code Copied!' : 'Copy Embed Code' }}</button>
                <p class="mb-0 px-2 small text-muted">{{ $t('menu.embedConfirmText') }} <a href="/site/terms">{{
                    $t('site.terms') }}</a></p>
            </div>
        </b-modal>

        <b-modal ref="ctxReport" id="ctx-report" hide-header hide-footer centered rounded size="sm"
            body-class="list-group-flush p-0 rounded">
            <p class="py-2 px-3 mb-0">
            ...
            <div class="list-group text-center">
                <div class="list-group-item rounded cursor-pointer font-weight-bold" @click="sendReport('spam')">{{
                    $t('menu.spam') }}</div>
                <div class="list-group-item rounded cursor-pointer font-weight-bold" @click="sendReport('sensitive')">{{
                    $t('menu.sensitive') }}</div>
                <div class="list-group-item rounded cursor-pointer font-weight-bold" @click="sendReport('abusive')">{{
                    $t('menu.abusive') }}</div>
                <div class="list-group-item rounded cursor-pointer font-weight-bold" @click="openCtxReportOtherMenu()">
                    {{
                        $t('common.other') }}</div>
                <!-- <div class="list-group-item rounded cursor-pointer" @click="ctxReportMenuGoBack()">Go Back</div> -->
                <div class="list-group-item rounded cursor-pointer text-lighter" @click="ctxReportMenuGoBack()">{{
                    $t('common.cancel') }}</div>
            </div>
        </b-modal>

        <b-modal ref="ctxReportOther" id="ctx-report-other" hide-header hide-footer centered rounded size="sm"
            body-class="list-group-flush p-0 rounded">
            <p class="py-2 px-3 mb-0">
            <div class="text-center font-weight-bold text-danger">{{ $t('menu.report') }}</div>
            <div class="small text-center text-muted">{{ $t('menu.selectOneOption') }}</div>
            ...
            <div class="list-group text-center">
                <div class="list-group-item rounded cursor-pointer font-weight-bold" @click="sendReport('underage')">{{
                    $t('menu.underageAccount') }}</div>
                <div class="list-group-item rounded cursor-pointer font-weight-bold" @click="sendReport('copyright')">{{
                    $t('menu.copyrightInfringement') }}</div>
                <div class="list-group-item rounded cursor-pointer font-weight-bold"
                    @click="sendReport('impersonation')">{{
                        $t('menu.impersonation') }}</div>
                <div class="list-group-item rounded cursor-pointer font-weight-bold" @click="sendReport('scam')">{{
                    $t('menu.scamOrFraud') }}</div>
                <div class="list-group-item rounded cursor-pointer text-lighter" @click="ctxReportOtherMenuGoBack()">{{
                    $t('common.cancel') }}</div>
            </div>
        </b-modal>

        <b-modal ref="ctxConfirm" id="ctx-confirm" hide-header hide-footer centered rounded size="sm"
            body-class="list-group-flush p-0 rounded">
            ...
            <div class="d-flex border-top btn-group btn-group-block rounded-0" role="group">
                <button type="button"
                    class="btn btn-outline-lighter border-left-0 border-top-0 border-bottom-0 border-right py-2"
                    style="color: rgb(0,122,255) !important;" @click.prevent="confirmModalCancel()">{{
                        $t('common.cancel')
                    }}</button>
                <button type="button" class="btn btn-outline-lighter border-0" style="color: rgb(0,122,255) !important;"
                    @click.prevent="confirmModalConfirm()">Confirm</button>
            </div>
        </b-modal>
