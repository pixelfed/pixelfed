    ...
    <div class="profile-sidebar-component">
        <div>
            <div class="d-block d-md-none">
                <div class="media user-card user-select-none">
                    <div style="position: relative">
                        <img
                            :src="profile.avatar"
                            class="avatar shadow cursor-pointer"
                            draggable="false"
                    ...
                    <div class="media-body">
                        ...
                        <p class="username" :class="{ remote: !profile.local }">
                            <a
                                ...
                                :href="profile.url"
                                class="primary"
                                >&commat;{{ profile.acct }}</a
                            >
                            <span v-else>&commat;{{ profile.acct }}</span>
                            <span v-if="profile.locked">
                                <i
                                    class="fal fa-lock ml-1 fa-sm text-lighter"
                                ></i>
                            </span>
                        </p>
                        <div class="stats">
            ...
            >
                ...
                <b-dropdown variant="link" right no-caret>
                    ...
                    <b-dropdown-item
                        ...
                        @click.prevent="goToOldProfile()"
                        >View in old UI
                    </b-dropdown-item>
                    <b-dropdown-item
                        href="#"
                        link-class="font-weight-bold"
                        @click.prevent="copyTextToClipboard(profile.url)"
                        >Copy Link
                    </b-dropdown-item>

                    <b-dropdown-item
                        v-if="profile.local"
                        :href="'/users/' + profile.username + '.atom'"
                        link-class="font-weight-bold"
                        >Atom feed
                    </b-dropdown-item>
                    ...
                    <div v-else>
                        ...
                        <b-dropdown-item
                            :href="'/i/web/direct/thread/' + profile.id"
                            link-class="font-weight-bold"
                        >
                            Direct Message
                        </b-dropdown-item>
                    ...
                    <div v-if="profile.id !== user.id">
                        ...
                        <b-dropdown-item
                            :href="'/i/report?type=user&id=' + profile.id"
                            link-class="text-danger font-weight-bold"
                            >Report
                        </b-dropdown-item>
                    </div>
                </b-dropdown>
            ...
            <div class="d-none d-md-block text-center">
                ...
                <p class="username" :class="{ remote: !profile.local }">
                    <a v-if="!profile.local" :href="profile.url" class="primary"
                        >&commat;{{ profile.acct }}</a
                    >
                    <span v-else>&commat;{{ profile.acct }}</span>
                    <span v-if="profile.locked">
                ...
                >
                    <span
                        ...
                        >{{ $t("profile.followYou") }}</span
                    >
                    <span
                        v-if="relationship.muting"
                        class="badge badge-dark p-1 ml-1"
                        >{{ $t("profile.muted") }}</span
                    >
                    <span
                        v-if="relationship.blocking"
                        class="badge badge-danger p-1 ml-1"
                        >{{ $t("profile.blocked") }}</span
                    >
                </p>
            </div>
            ...
            <div class="d-none d-md-block stats py-2">
                <div class="d-flex justify-content-between">
                    <button
                        ...
                        @click="toggleTab('index')"
                    >
                        <strong :title="profile.statuses_count">{{
                            formatCount(profile.statuses_count)
                        }}</strong>
                    ...
                    <button
                        ...
                        @click="toggleTab('followers')"
                    >
                        <strong :title="profile.followers_count">{{
                            formatCount(profile.followers_count)
                        }}</strong>
                    ...
                    <button
                        ...
                        @click="toggleTab('following')"
                    >
                        <strong :title="profile.following_count">{{
                            formatCount(profile.following_count)
                        }}</strong>
            ...
            <div class="d-flex align-items-center mb-3 mb-md-0">
                <div v-if="user.id === profile.id" style="flex-grow: 1">
                    ...
                    <a
                        v-if="!profile.locked"
                        class="btn btn-light font-weight-bold btn-block follow-btn mt-md-n4"
                ...
                <div
                    v-else-if="
                        profile.hasOwnProperty('moved') && profile.moved.id
                    "
                    style="flex-grow: 1"
                >
                    <div class="card shadow-none rounded-lg mb-3 bg-danger">
                        <div class="card-body">
                            <div
                                class="d-flex align-items-center ft-std text-white mb-2"
                            >
                                <i
                                    class="far fa-exclamation-triangle mr-2 text-white"
                                ></i>
                ...
                <div v-else-if="profile.locked" style="flex-grow: 1">
                    ...
                    >
                        ...
                        <p
                            v-if="relationship.blocking"
                            class="mt-n4 text-lighter"
                            style="font-size: 11px"
                        >
                            You need to unblock this account before you can
                            request to follow.
                        </p>
                    </template>
                    ...
                    <div v-else-if="relationship.requested">
                        ...
                        <p class="small font-weight-bold text-center mt-n4">
                            <a href="#" @click.prevent="cancelFollowRequest()"
                                >Cancel Follow Request</a
                            >
                        </p>
                    </div>
            ...
            <div
                v-if="profile.note && renderedBio && renderedBio.length"
                class="bio-wrapper card shadow-none"
            >
                <div class="card-body">
                    <div class="bio-body">
            ...
            <div class="d-none d-md-block card card-body shadow-none py-2">
                <p v-if="profile.website" class="small">
                    ...
                    <span>
                        <a :href="profile.website" class="font-weight-bold">{{
                            profile.website
                        }}</a>
                    </span>
                </p>
                    ...
                    <span v-else>
                        ...
                        <span class="float-right primary">
                            <i
                                ...
                                v-b-tooltip.hover
                                title="This user is from a remote server and may have created their account before this date"
                            ></i>
                        </span>
                    </span>
                </p>
            ...
            <div class="d-none d-md-flex flex-wrap sidebar-sitelinks">
                ...
                <a v-if="showLegalNoticeLink" href="/site/legal-notice">{{
                    $t("navmenu.legalNotice")
                }}</a>
            </div>

            <div class="d-none d-md-block sidebar-attribution">
                <a href="https://pixelfed.org" class="font-weight-bold"
                    >Powered by Pixelfed</a
                >
            </div>
        </div>
    ...
    methods: {
        getDisplayName() {
            ...
            if (dn.includes(":")) {
                ...
                let un = dn.replaceAll(re, function (em) {
                    ...
                    return emoji.length
                        ? `<img draggable="false" class="emojione custom-emoji" alt="${emoji[0].shortcode}" title="${emoji[0].shortcode}" src="${emoji[0].url}" data-original="${emoji[0].url}" data-static="${emoji[0].static_url}" width="16" height="16" onerror="this.onerror=null;this.src='/storage/emoji/missing.png';" />`
                        : em;
                });
                return un;
            } else {
        ...
        toggleTab(tab) {
            ...
            if (["followers", "following"].includes(tab)) {
                this.$router.push(
                    "/i/web/profile/" + this.profile.id + "/" + tab
                );
                return;
            } else {
        ...
        setBio() {
            ...
            if (this.profile.local) {
                ...
                el.querySelectorAll('a[class*="hashtag"]').forEach((elr) => {
                ...
                el.querySelectorAll(
                    'a:not(.hashtag)[class*="mention"], a:not(.hashtag)[class*="list-slug"]'
                ).forEach((elr) => {
        ...
        handleMute() {
            ...
            let url =
                this.relationship.muting == true ? "/i/unmute" : "/i/mute";
            axios
                .post(url, {
                    type: "user",
        ...
        handleBlock() {
            ...
            let url =
                this.relationship.blocking == true ? "/i/unblock" : "/i/block";
            axios
                .post(url, {
                    type: "user",
                    item: this.profile.id,
