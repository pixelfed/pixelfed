<template>
    <nav class="metro-nav navbar navbar-expand navbar-light navbar-laravel sticky-top shadow-none py-1">
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse">
                <div class="navbar-nav ml-auto">
                    <autocomplete class="searchbox" :search="autocompleteSearch" :placeholder="$t('navmenu.search')"
                        aria-label="Search" :get-result-value="getSearchResultValue" :debounceTime="700" submit-on-enter
                        @submit="onSearchSubmit" ref="autocomplete">

                        <template #result="{ result, props }">
                            <li v-bind="props" class="autocomplete-result sr">
                                <div v-if="result.s_type === 'account'" class="media align-items-center my-0">
                                    <img :src="result.avatar" width="40" height="40" class="sr-avatar"
                                        style="border-radius: 40px"
                                        onerror="this.src='/storage/avatars/default.png?v=0';this.onerror=null;">
                                    <div class="media-body sr-account">
                                        <div class="sr-account-acct"
                                            :class="{ compact: result.acct && result.acct.length > 24 }">
                                            &commat;{{ result.acct }}
                                            <b-button v-if="result.locked" v-b-tooltip.html title="Private Account"
                                        ...
                                        <template v-else>
                                            <template v-if="result.local">
                                                <div class="sr-account-stats">
                                                    <div v-if="result.followers_count"
                                                        class="sr-account-stats-followers font-weight-bold">
                                                        <span>{{ formatCount(result.followers_count) }}</span>
                                                        <span>Followers</span>
                                                    </div>
                                                    <div v-if="result.followers_count && result.statuses_count">·</div>
                                                    <div v-if="result.statuses_count"
                                                        class="sr-account-stats-statuses font-weight-bold">
                                            ...
                                            <template v-else>
                                                <div class="sr-account-stats">
                                                    ...
                                                    <b-button v-b-tooltip.html
                                                        :title="'Joined ' + timeAgo(result.created_at) + ' ago'"
                                                        variant="link" size="sm" class="sr-account-stats-statuses p-0">
                                                        <i class="far fa-clock fa-sm"></i>
                                                        <span class="font-weight-bold">{{ timeAgo(result.created_at)
                                                            }}</span>
                                                    </b-button>
                                                </div>
                                            </template>
                                ...
                                <div v-else-if="result.s_type === 'hashtag'" class="media align-items-center my-0">
                                    ...
                                    <div class="media-body sr-tag">
                                        <div class="sr-tag-name"
                                            :class="{ compact: result.name && result.name.length > 26 }">
                                            #{{ result.name }}
                                        </div>
                                ...
                                <div v-else-if="result.s_type === 'status'" class="media align-items-center my-0">
                                    <img :src="result.account.avatar" width="40" height="40" class="sr-avatar"
                                        style="border-radius: 40px"
                                        onerror="this.src='/storage/avatars/default.png?v=0';this.onerror=null;">

                                    <div class="media-body sr-post">
                                        <div class="sr-post-acct"
                                            :class="{ compact: result.acct && result.acct.length > 26 }">
                    ...
                    <ul class="navbar-nav align-items-center">
                        <li class="nav-item dropdown ml-2">
                            <a id="navbarDropdown" class="nav-link dropdown-toggle" href="#" role="button"
                                data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" title="User Menu">
                                <i class="d-none far fa-user fa-lg text-dark"></i>
                                <span class="sr-only">User Menu</span>
                                <img :src="user.avatar" class="nav-avatar rounded-circle border shadow" width="30"
                                    height="30"
                                ...
                                <ul class="nav flex-column">
                                    <li class="nav-item nav-icons">
                                        <div class="d-flex justify-content-between align-items-center">
                                            ...
                                            <router-link v-if="hasLocalTimeline" class="nav-link text-center"
                                                :to="{ name: 'timeline', params: { scope: 'local' } }">
                                                <div class="icon text-lighter"><i class="fas fa-stream fa-lg"></i></div>
                                                <div class="small">{{ $t('navmenu.localFeed') }}</div>
                                            </router-link>

                                            <router-link v-if="hasNetworkTimeline" class="nav-link text-center"
                                                :to="{ name: 'timeline', params: { scope: 'global' } }">
                                    ...
                                    <li class="nav-item nav-icons">
                                        <div class="d-flex justify-content-between align-items-center">
                                            ...
                                            <router-link class="nav-link text-center px-3"
                                                :to="'/i/web/profile/' + user.id">
                                                <div class="icon text-lighter">
        ...
        <b-modal ref="uis" hide-footer centered body-class="p-0 ui-menu" :title="$t('navmenu.appearance')">
...
export default {
    ...
    methods: {
        ...
        getSearchResultValue(result) {
            // The lib writes this return value straight into the <input>
            // when a result is picked with Enter or Tab, so it has to be
            // a string. Returning the result object is what produced
            // "[object Object]" in the search box on keyboard selection.

            // mounted() calls setValue() with the raw ?q= string
            ...
            if (result.s_type === 'account') {
                return '@' + result.acct;
            }

            if (result.s_type === 'hashtag') {
                return '#' + result.name;
            }

            // Status results have no sensible text value, and filling in
            // the post url would trigger the url resolve + redirect on the
            // next focus, so just keep whatever was typed
            const ac = this.$refs.autocomplete;
            return ac && typeof ac.value === 'string' ? ac.value : '';
        },

        onSearchSubmit(result) {
            // Guard against Enter with nothing highlighted
            if (!result || !result.s_type) {
                return;
            }
            ...
            if (ac) {
                ...
                // With submit-on-enter the lib never clears the result it
                // remembered, so a later bare Enter would re-submit it
                if (ac.core) {
                    ac.core.selectedResult = null;
                }
            }
...
.metro-nav {
    ...
    .autocomplete {
        ...
        &-result {
            ...
            // Hover + keyboard cursor. Built on theme vars instead of a
            // .force-dark-mode override so it also works when Appearance
            // is "Auto" and dark comes from prefers-color-scheme (body has
            // no class in that case, so .force-dark-mode never matches).
            &:hover,
            &[aria-selected="true"] {
                background-color: var(--comment-bg);
            }

            // Only the real keyboard cursor gets the bar, so it stays
            // distinguishable from a row the mouse happens to rest on.
            // Inset shadow rather than border-left to avoid layout shift.
            &[aria-selected="true"] {
