...
@section('section')
  ...
  <div class="title">
    <h3 class="font-weight-bold">{{ __('Privacy Settings')}}</h3>
  </div>
  <hr>
  <div class="form-group pb-1">
    <p>
      <a class="btn btn-outline-secondary py-0 font-weight-bold" href="{{route('settings.privacy.muted-users')}}">{{ __('Muted Users')}}</a>
  ...
  <form method="post">
    ...
    <div class="form-check pb-3">
      ...
      <p class="text-muted small help-text">{{ __('When your account is private, only people you approve can see your photos and videos on pixelfed. Your existing followers won\'t be affected.')}}</p>
    </div>
    <div class="form-check pb-3">
      ...
      <p class="text-muted small help-text">{{ __('When your account is visible to search engines, your information can be crawled and stored by search engines.')}}</p>
    </div>
    ...
    {{-- <div class="form-check pb-3">
      ...
      <p class="text-muted small help-text">{{ __('When this option is enabled, your profile and posts are used for discover recommendations. Only public profiles and posts are used.')}}</p>
    </div> --}}
    <div class="form-check pb-3">
      ...
      <p class="text-muted small help-text">{{ __('If selected, you will be able to receive messages and notifications from any user even if you do not follow them.')}}</p>
    </div>
    {{-- <div class="form-check pb-3">
      ...
      <p class="text-muted small help-text">{{ __('This prevents posts with potentially sensitive content from displaying in your search results.')}}</p>
    </div> --}}
    {{-- <div class="form-check pb-3">
      ...
      <p class="text-muted small help-text">{{ __('Use this to eliminate search results from accounts you've blocked or muted.')}}</p>
    </div>
    <div class="form-check pb-3">
      ...
      <p class="text-muted small help-text">{{ __('Show all media, including potentially sensitive content.')}}</p>
    </div> --}}
    ...
    <div class="form-check pb-3">
      ...
      <p class="text-muted small help-text">{{ __('Display follower count on profile')}}</p>
    </div>
    ...
    <div class="form-check pb-3">
      ...
      <p class="text-muted small help-text">{{ __('Display following count on profile')}}</p>
    </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">{{ __('Confirm this action')}}</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body p-3">
        <p class="font-weight-bold">{{ __('Please select the type of private account you would like:')}}</p>
        <div>
          ...
          <div class="form-check">
            ...
            {{ __('Only followers that have followed you for atleast')}} <select name="pfDuration">
                  <option value="60">1 {{ __('hour')}}</option>
                  <option value="1440">1 {{ __('day')}}</option>
                  <option value="20160">2 {{ __('weeks')}}</option>
                  <option value="43200">1 {{ __('month')}}</option>
                  <option value="259200">6 {{ __('months')}}</option>
                  <option value="525600">1 {{ __('year')}}</option>
                </select>
            </label>
          </div>
          <div class="form-check">
          ...
          <div class="form-check pt-3">
            ...
            {{ __('Allow new follow requests')}}
            </label>
          </div>
          <div class="form-check">
            <input class="form-check-input" type="checkbox" name="blockNotifications" id="chk4">
            <label class="form-check-label" for="chk4">
            {{ __('Block notifications from accounts I don't follow')}}
            </label>
          </div> --}}
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-outline-secondary font-weight-bold py-0" data-dismiss="modal">{{ __('Cancel')}}</button>
        <button type="button" class="btn btn-primary font-weight-bold py-0" id="modal_confirm">{{ __('Save')}}</button>
      </div>
    </div>
  </div>
</div>
